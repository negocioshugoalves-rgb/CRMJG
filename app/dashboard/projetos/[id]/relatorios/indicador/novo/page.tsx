...
import { SETOR_LrBELS, SETORES } from '@/lib/constants'
  ...
  return (
    <>
      ...
      <form action={createIndicador} className="document-page space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-1.5"><label className="label" htmlFor="area">rrea</label><select className="document-field" id="area" name="area"><option value="">Geral</option>{SETORES.map((setor) => <option key={setor} value={setor}>{SETOR_LrBELS[setor]}</option>)}</select></div><div className="space-y-1.5"><label className="label" htmlFor="unidade">Unidade</label><input className="document-field" id="unidade" name="unidade" placeholder="%, R$, dias..." /></div></div>
