...
export default async function NovoIndicadorPage({ params }: { params: { id: string } }) {
  ...
  const projeto = data as Projeto & { empresas?: Pick<Empresa, 'id' | 'nome'> | null }

  return (
    <>
      <PageHeader title={`Novo indicador - ${projeto.nome}`} description="Cadastre uma métrica para acompanhar a evolução do projeto." />
      <form action={createIndicador} className="document-page space-y-6">
        <input type="hidden" name="projeto_id" value={projeto.id} />
        <input type="hidden" name="redirect_to" value={`/dashboard/projetos/${projeto.id}/relatorios`} />
        ...
        <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-1.5"><label className="label" htmlFor="area">Área</label><select className="document-field" id="area" name="area"><option value="">Geral</option>{SETORES.map((setor) => <option key={setor} value={setor}>{SETOR_LABELS[setor]}</option>)}</select></div><div className="space-y-1.5"><label className="label" htmlFor="unidade">Unidade</label><input className="document-field" id="unidade" name="unidade" placeholder="%, R$, dias..." /></div></div>
