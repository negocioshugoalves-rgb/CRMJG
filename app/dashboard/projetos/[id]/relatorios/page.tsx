...
export default async function RelatoriosProjetoPage({ params }: { params: { id: string } }) {
  ...
  return (
    <>
      <PageHeader title={`Relatorios - ${projeto.nome}`} description={`Acompanhamento do projeto${projeto.empresas?.nome ? ` de ${projeto.empresas.nome}` : ''}.`} />
      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <div><h3 className="mb-4 text-lg font-bold text-brand-ink">Relatorios do projeto</h3>{relatorios.length ? <div className="grid gap-4">{relatorios.map((relatorio) => <article className="panel p-5" key={relatorio.id}><h4 className="font-semibold text-brand-ink">{relatorio.titulo}</h4><p className="mt-1 text-sm text-stone-500">{relatorio.periodo_inicio || '-'} ate {relatorio.periodo_fim || '-'}</p><p className="mt-3 text-sm leading-6 text-stone-700"><strong>Executado:</strong> {relatorio.atividades_realizadas}</p>{relatorio.resultados_obtidos ? <p className="mt-3 text-sm leading-6 text-stone-700"><strong>Resultados:</strong> {relatorio.resultados_obtidos}</p> : null}{relatorio.proximos_passos ? <p className="mt-3 rounded-md bg-stone-50 p-3 text-sm leading-6 text-stone-700"><strong>Proximos passos:</strong> {relatorio.proximos_passos}</p> : null}</article>)}</div> : <EmptyState message="Nenhum relatorio registrado para este projeto." />}</div>
      </section>
    </>
  )
}
