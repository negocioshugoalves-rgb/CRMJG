  ...
  return (
    <>
      ...
      <section className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <form action={createProjeto} className="panel space-y-4 p-5">
          ...
          <div className="space-y-1.5"><label className="label" htmlFor="proposta_id">Proposta aceita</label><select className="field" id="proposta_id" name="proposta_id"><option value="">Sem vinculo</option>{propostas.map((proposta) => <option key={proposta.id} value={proposta.id}>{proposta.titulo}</option>)}</select></div>
          <div className="space-y-1.5"><label className="label" htmlFor="nome">Nome do projeto</label><input className="field" id="nome" name="nome" required /></div>
          <div className="space-y-1.5"><label className="label" htmlFor="objetivo">Objetivo</label><textarea className="field min-h-24" id="objetivo" name="objetivo" /></div>
          <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-1.5"><label className="label" htmlFor="data_inicio">Inicio</label><input className="field" id="data_inicio" name="data_inicio" type="date" /></div><div className="space-y-1.5"><label className="label" htmlFor="data_fim_prevista">Fim previsto</label><input className="field" id="data_fim_prevista" name="data_fim_prevista" type="date" /></div></div>
          <div className="space-y-1.5"><label className="label" htmlFor="status">Status</label><select className="field" id="status" name="status">{statusProjetos.map((status) => <option key={status} value={status}>{STATUS_PROJETO_LABELS[status]}</option>)}</select></div>
