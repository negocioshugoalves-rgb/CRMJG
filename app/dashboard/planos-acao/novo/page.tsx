...
export default async function NovaAcaoPage() {
  ...
  return (
    <>
      <PageHeader title="Nova ação" description="Selecione a empresa e documente o plano de ação." />
      <form action={createPlanoAcao} className="document-page space-y-6">
        ...
        <div className="space-y-1.5"><label className="label" htmlFor="titulo">Título da ação</label><input className="document-field" id="titulo" name="titulo" required /></div>
        <div className="space-y-1.5"><label className="label" htmlFor="descricao">Descrição</label><textarea className="document-field min-h-40" id="descricao" name="descricao" required /></div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row"><button className="btn-primary" disabled={!empresas.length} type="submit">Salvar ação</button><Link className="btn-secondary" href="/dashboard/planos-acao">Cancelar</Link></div>
      </form>
    </>
  )
}
