      ...
      <section className="space-y-8">
        <form action={createPlanoAcao} className="document-page space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5"><label className="label" htmlFor="setor">Area</label><select className="document-field" id="setor" name="setor">{SETORES.map((setor) => <option key={setor} value={setor}>{SETOR_LABELS[setor]}</option>)}</select></div>
            <div className="space-y-1.5"><label className="label" htmlFor="prioridade">Prioridade</label><select className="document-field" id="prioridade" name="prioridade">{prioridades.map((p) => <option key={p} value={p}>{PRIORIDADE_LABELS[p]}</option>)}</select></div>
          </div>
          ...
          <div className="space-y-1.5"><label className="label" htmlFor="descricao">O que sera feito</label><textarea className="document-field min-h-24" id="descricao" name="descricao" required /></div>
          <div className="space-y-1.5"><label className="label" htmlFor="justificativa">Por que sera feito</label><textarea className="document-field min-h-20" id="justificativa" name="justificativa" /></div>
          ...
          <div className="space-y-1.5"><label className="label" htmlFor="resultado_esperado">Resultado esperado</label><textarea className="document-field min-h-20" id="resultado_esperado" name="resultado_esperado" /></div>
