...
function numberValue(formData: FormData, key: string) { const value = text(formData, key); return value ? Number(value.replace(',', '.')) : null }
...
export async function createIndicador(formData: FormData) {
  ...
  if (!projetoId || !nome) throw new Error('Selecione o projeto e informe o indicador.')
...
export async function createRelatorio(formData: FormData) {
  ...
  if (!projetoId || !titulo || !atividades) throw new Error('Preencha projeto, titulo e atividades.')
