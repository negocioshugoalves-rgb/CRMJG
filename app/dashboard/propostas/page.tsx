    ...
    <>
      ...
      <section className="grid gap-6 xl:grid-cols-[440px_1fr]">
        <form action={createProposta} className="panel space-y-4 p-5">
          ...
          <div className="space-y-1.5"><label className="label" htmlFor="resumo_diagnostico">Resumo do diagnostico</label><textarea className="field min-h-24" id="resumo_diagnostico" name="resumo_diagnostico" /></div>
          <div className="space-y-1.5"><label className="label" htmlFor="descricao">Escopo / plano de acao proposto</label><textarea className="field min-h-28" id="descricao" name="descricao" /></div>
          <div className="space-y-1.5"><label className="label" htmlFor="metodologia">Metodologia</label><textarea className="field min-h-24" id="metodologia" name="metodologia" /></div>
          <div className="space-y-1.5"><label className="label" htmlFor="cronograma">Cronograma</label><textarea className="field min-h-20" id="cronograma" name="cronograma" /></div>
          <div className="space-y-1.5"><label className="label" htmlFor="condicoes_comerciais">Condicoes comerciais</label><textarea className="field min-h-20" id="condicoes_comerciais" name="condicoes_comerciais" /></div>
          <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-1.5"><label className="label" htmlFor="valor">Investimento</label><input className="field" id="valor" name="valor" type="number" step="0.01" /></div><div className="space-y-1.5"><label className="label" htmlFor="status">Status</label><select className="field" id="status" name="status">{statusProposta.map((status) => <option key={status} value={status}>{STATUS_PROPOSTA_LABELS[status]}</option>)}</select></div></div>
          <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-1.5"><label className="label" htmlFor="data_envio">Envio</label><input className="field" id="data_envio" name="data_envio" type="date" /></div><div className="space-y-1.5"><label className="label" htmlFor="data_validade">Validade</label><input className="field" id="data_validade" name="data_validade" type="date" /></div></div>
