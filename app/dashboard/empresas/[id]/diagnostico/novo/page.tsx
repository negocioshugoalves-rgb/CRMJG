...
export default async function NovoDiagnosticoPage({ params }: { params: { id: string } }) {
  ...
  return (
    <>
      <PageHeader title={`Novo diagnostico - ${empresa.nome}`} description="Preencha a analise em formato de documento, com espaco para textos longos." />
      ...
      <form action={createDiagnostico} className="document-page space-y-6">
        ...
        <div className="space-y-1.5"><label className="label" htmlFor="recomendacoes">Recomendacoes</label><textarea className="document-field min-h-32" id="recomendacoes" name="recomendacoes" /></div>
        <div className="flex flex-col gap-3 sm:flex-row">
          <button className="btn-primary" type="submit">Salvar diagnostico</button>
