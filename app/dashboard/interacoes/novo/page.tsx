...
export default async function NovaInteracaoPage() {
  ...
  return (
    <>
      <PageHeader title="Nova interacao" description="Registre um contato, reuniao, ligacao ou proximo acompanhamento." />
      <form action={createInteracao} className="document-page space-y-6">
        ...
        <div className="space-y-1.5"><label className="label" htmlFor="descricao">Descricao</label><textarea className="document-field min-h-48" id="descricao" name="descricao" required /></div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row"><button className="btn-primary" disabled={!empresas.length} type="submit">Salvar interacao</button><Link className="btn-secondary" href="/dashboard/interacoes">Cancelar</Link></div>
      </form>
    </>
  )
}
