...
type DiagnosticoComEmpresa = Diagnostico & {
...
export default async function DiagnosticosPage() {
  ...
  return (
    <>
      <PageHeader
        title="Diagnosticos"
        description="Registre pareceres por setor com pontos fortes, pontos fracos e recomendacoes para orientar a proposta."
      />
      ...
      <section className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <form action={createDiagnostico} className="panel space-y-4 p-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-brand-ink">Novo parecer</h3>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="label" htmlFor="parecer">
              Parecer
            </label>
            <textarea className="field min-h-28" id="parecer" name="parecer" required />
          </div>

          <div className="space-y-1.5">
            <label className="label" htmlFor="pontos_fortes">
              Pontos fortes
            </label>
            <textarea className="field min-h-20" id="pontos_fortes" name="pontos_fortes" />
          </div>

          <div className="space-y-1.5">
            <label className="label" htmlFor="pontos_fracos">
              Pontos fracos
            </label>
            <textarea className="field min-h-20" id="pontos_fracos" name="pontos_fracos" />
          </div>

          <div className="space-y-1.5">
            <label className="label" htmlFor="recomendacoes">
              Recomendacoes
            </label>
            <textarea className="field min-h-20" id="recomendacoes" name="recomendacoes" />
          </div>
          ...
          <div className="grid gap-4">
            {diagnosticos.map((diagnostico) => (
              <article className="panel p-5" key={diagnostico.id}>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h3 className="font-semibold text-brand-ink">
                      {diagnostico.empresas?.nome || 'Empresa'}
                    </h3>
                    <p className="text-sm text-stone-500">{SETOR_LABELS[diagnostico.setor]}</p>
                  </div>
                ...
                <p className="mt-4 text-sm leading-6 text-stone-700">{diagnostico.parecer}</p>
                {diagnostico.recomendacoes ? (
                  <p className="mt-3 rounded-md bg-stone-50 p-3 text-sm text-stone-700">
                    <strong>Recomendacao:</strong> {diagnostico.recomendacoes}
                  </p>
                ) : null}
              </article>
            ))}
